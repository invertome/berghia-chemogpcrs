...
def test_phylo_and_lse_divergence_gated_by_tree_membership() -> None:
    """o98: phylo/lse_divergence are class-A chemoreceptor-tree signals, so they must
    ...
    # Bead hf3u: lse_divergence is gated by a STRICTER flag than tree membership.
    # It also needs a derivable percentile threshold (LSE_DIVERGENCE_AVAILABLE);
    ...
    assert "'lse_divergence': row.get('lse_divergence_score_norm') if row.get('has_lse_divergence_data') else None" in body
