...
class TestReadTargetsDropReason:
    """Rows with a non-empty drop_reason must be excluded from targets even
    when they carry a valid accession; rows with an empty drop_reason and a
    valid accession must still be included.
    """

    def _write_gi(self, path: Path, rows: list[dict]) -> None:
        cols = (
    ...
    def test_whitespace_only_drop_reason_treated_as_empty(self, tmp_path: Path) -> None:
